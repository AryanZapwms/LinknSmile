...
// Bunny.net's pull zone hostname, if configured (see lib/storage/bunny.ts).
// Allowed unconditionally alongside res.cloudinary.com below — old
// Cloudinary-hosted images must keep resolving regardless of which
// provider IMAGE_PROVIDER currently selects for new uploads.
const bunnyPullZoneHostname = process.env.BUNNY_PULL_ZONE_HOSTNAME;
...
const nextConfig = {
  ...
  images: {
    ...
    remotePatterns: [
      ...
      ...(bunnyPullZoneHostname ? [{ protocol: "https", hostname: bunnyPullZoneHostname }] : []),
    ],
  },
