...
export default function ImagesPage() {
  ...
  const getFilteredImages = (folder: string, used: boolean) => {
    return images.filter(
      (img) => (folder === "all" || img.folder === folder) && img.isUsed === used
    );
  };
  ...
  return (
    <div className="container mx-auto space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Image Management</h1>
          <p className="text-muted-foreground">Manage images by folder and remove unused files</p>
        </div>
                  ...
                  {getFilteredImages(folder, true).map((image) => (
                    <Card key={image.path} className="overflow-hidden">
                      ...
                      <CardContent className="p-3">
                        <div className="space-y-2">
                          <div className="flex items-center justify-between">
                            <Badge className={getFolderColor(image.folder)}>{image.folder}</Badge>
                            <Badge variant="secondary" className="bg-green-100 text-green-800">
                              Used
                            </Badge>
                          </div>
              ...
              <div>
                <div className="mb-4 flex items-center justify-between">
                  ...
                  <Button
                    ...
                    className="flex items-center gap-2"
                  >
                    {getFilteredImages(folder, false).every((img) =>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                  {getFilteredImages(folder, false).map((image) => (
                    <Card
                      key={image.path}
                      className={`relative overflow-hidden border-orange-200 ${
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the selected images from
              the server.
