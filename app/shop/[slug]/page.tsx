    ...
    <main className="min-h-screen bg-stone-50">
      ...
      <div className="relative h-44 w-full overflow-hidden bg-gradient-to-br from-amber-100 via-amber-50 to-stone-100 md:h-60">
        ...
        <div className="absolute top-4 start-4">
          ...
          >
            <ArrowLeft className="h-4 w-4 rtl:rotate-180" /> {t("backToProducts")}
          </Link>
        </div>
      </div>
            ...
            <div className="mb-1 flex flex-wrap items-center gap-2">
              ...
              {shop.isApproved && (
                <span className="flex items-center gap-1 rounded-full border border-amber-200 bg-amber-50 px-2.5 py-1 text-xs font-bold text-amber-700">
                  <ShieldCheck className="h-3.5 w-3.5" /> {t("verifiedSeller")}
                </span>
              )}
              ...
              <div className="flex items-center gap-1.5 rounded-full border border-stone-200 bg-stone-50 px-2.5 py-1">
                ...
                <span className="text-xs font-medium text-stone-500">{t("saveSeller")}</span>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm text-stone-500">
              ...
              {shop.ratings?.count > 0 && (
                <span className="flex items-center gap-1">
                  ...
                  {t("ratingWithReviews", {
                    rating: shop.ratings.average.toFixed(1),
          ...
          <div className="flex shrink-0 gap-3">
            {madeInIndia > 0 && (
              <div className="rounded-xl border border-orange-100 bg-orange-50 px-4 py-2 text-center">
                ...
                <p className="text-[10px] font-medium text-orange-500">{t("madeInIndia")}</p>
              </div>
            )}
            {international > 0 && (
              <div className="rounded-xl border border-blue-100 bg-blue-50 px-4 py-2 text-center">
                ...
                <p className="text-[10px] font-medium text-blue-500">{t("international")}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_280px]">
          ...
          <section>
            <div className="mb-5 flex items-center justify-between">
              <h2 className="text-lg font-bold text-stone-800">{t("allProducts")}</h2>
            ...
            {products.length === 0 ? (
              <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-stone-200 bg-white py-20">
                ...
                <p className="mb-1 font-bold text-stone-600">{t("noProductsTitle")}</p>
                <p className="text-sm text-stone-400">{t("noProductsBody")}</p>
              </div>
            ) : (
          ...
          <aside className="space-y-5">
            ...
            <div className="rounded-2xl border border-stone-100 bg-white p-5 shadow-sm">
              <h3 className="mb-3 text-base font-bold text-stone-800">{t("aboutSellerTitle")}</h3>
            ...
            <div className="rounded-2xl bg-gradient-to-br from-amber-400 to-amber-500 p-5 text-white shadow-lg shadow-amber-100">
              <div className="mb-2 flex items-center gap-2">
                ...
                <h4 className="text-sm font-bold">{t("trustedTitle")}</h4>
              </div>
              <p className="text-xs leading-relaxed text-white/85">{t("trustedBody")}</p>
            </div>

            {/* Browse more */}
            <div className="rounded-2xl border border-stone-100 bg-white p-5 shadow-sm">
              <h3 className="mb-3 text-sm font-bold text-stone-700">{t("browseMoreTitle")}</h3>
