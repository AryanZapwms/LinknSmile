...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!shopId || !action || !reason) {
      return withCORS(
        NextResponse.json({ message: "shopId, action, and reason are required" }, { status: 400 })
      );
    }
    ...
    if (action === "freeze") {
      ...
      return withCORS(
        NextResponse.json({ success: true, message: `Wallet frozen. Reason: ${reason}` })
      );
    }
    ...
    if (action === "unfreeze") {
      ...
      return withCORS(
        NextResponse.json({ success: true, message: `Wallet unfrozen. Reason: ${reason}` })
      );
    }
    ...
    return withCORS(
      NextResponse.json({ message: 'Invalid action. Use "freeze" or "unfreeze".' }, { status: 400 })
    );
  } catch (error: any) {
