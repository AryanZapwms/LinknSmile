...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    if (!imagePath) {
      return withCORS(
        NextResponse.json({ success: false, error: "Image path is required" }, { status: 400 })
      );
    }
    ...
    if (!fs.existsSync(fullImagePath)) {
      return withCORS(
        NextResponse.json({ success: false, error: "Image file not found" }, { status: 404 })
      );
    }
    ...
    return withCORS(
      NextResponse.json({ success: false, error: "Failed to delete image" }, { status: 500 })
    );
  }
}
