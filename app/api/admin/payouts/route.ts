  ...
  try {
    ...
    if (action === "approve") {
      if (payout.status !== "REQUESTED") {
        return withCORS(
          NextResponse.json({ message: "Only REQUESTED payouts can be approved" }, { status: 400 })
        );
      }
      nextStatus = "APPROVED";
      ...
      if (!transactionId) {
        return withCORS(
          NextResponse.json({ message: "Bank transaction ID is required" }, { status: 400 })
        );
      }
      if (!["APPROVED", "PROCESSING"].includes(payout.status)) {
        return withCORS(
          NextResponse.json(
            { message: "Only APPROVED or PROCESSING payouts can be completed" },
            { status: 400 }
          )
        );
      }
      ...
      if (!failureReason) {
        return withCORS(
          NextResponse.json({ message: "Rejection reason is required" }, { status: 400 })
        );
      }
      if (payout.status === "COMPLETED") {
        return withCORS(
          NextResponse.json({ message: "Cannot reject a completed payout" }, { status: 400 })
        );
      }
      nextStatus = "FAILED";
