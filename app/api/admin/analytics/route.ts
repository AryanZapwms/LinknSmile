...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const previousAverageOrderValue =
      overviewTotalOrders > 0 ? (overviewRevenueAgg[0]?.total || 0) / overviewTotalOrders : 0;
    ...
    const highValueThreshold = customerAggregates.length ? totalClv / customerAggregates.length : 0;
    ...
    const averageOrderValueCurrent = overviewTotalOrders
      ? (overviewRevenueAgg[0]?.total || 0) / overviewTotalOrders
      : 0;
    ...
    return withCORS(
      NextResponse.json({
        ...
        paymentAnalytics,
        timeAnalytics: {
          hourly: hourlyOrders.map((item, index) => ({
            hour: index,
            orders: item.orders,
            revenue: item.revenue,
          })),
          weekday: weekdayOrders.map((item, index) => ({
            weekday: index,
            orders: item.orders,
            revenue: item.revenue,
          })),
