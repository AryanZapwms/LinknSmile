...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return withCORS(
      NextResponse.json({ message: "Failed to fetch order", error: error.message }, { status: 500 })
    );
  }
}
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (orderStatus === "cancelled" && !cancellationReason?.trim()) {
      return withCORS(
        NextResponse.json({ message: "Please provide a reason for cancellation" }, { status: 400 })
      );
    }
