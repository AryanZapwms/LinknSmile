...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!shopId) return withCORS(NextResponse.json({ message: 'Shop not found' }, { status: 404 }));
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!amount || typeof amount !== 'number' || amount <= 0) {
      return withCORS(NextResponse.json({ message: 'Invalid amount' }, { status: 400 }));
    }

    const shop = await Shop.findById(shopId);
    if (!shop) return withCORS(NextResponse.json({ message: 'Shop not found' }, { status: 404 }));
    ...
    if (!wallet) {
      return withCORS(NextResponse.json({ message: 'Wallet not found. No earnings yet.' }, { status: 400 }));
    }
    ...
    if (wallet.status === 'CLOSED') {
      return withCORS(NextResponse.json({ message: 'Your account is closed.' }, { status: 400 }));
    }
