...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    if (!accountHolderName?.trim()) {
      return withCORS(
        NextResponse.json({ message: "Account holder name is required" }, { status: 400 })
      );
    }
    if (!bankName?.trim()) {
      return withCORS(NextResponse.json({ message: "Bank name is required" }, { status: 400 }));
    }
    if (!accountNumber?.trim()) {
      return withCORS(
        NextResponse.json({ message: "Account number is required" }, { status: 400 })
      );
    }
    ...
    if (!/^\d+$/.test(cleanAccountNumber)) {
      return withCORS(
        NextResponse.json({ message: "Account number must contain digits only" }, { status: 400 })
      );
    }
    ...
    if (upiId?.trim()) {
      ...
      if (!upiRegex.test(upiId.trim())) {
        return withCORS(
          NextResponse.json({ message: "Invalid UPI ID format (e.g. name@upi)" }, { status: 400 })
        );
      }
    }
