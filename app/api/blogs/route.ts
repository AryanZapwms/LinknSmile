...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return withCORS(NextResponse.json({ error: "Failed to fetch blogs" }, { status: 500 }));
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!title || !slug || !content) {
      return withCORS(
        NextResponse.json({ error: "Title, slug, and content are required" }, { status: 400 })
      );
    }
    ...
    return withCORS(NextResponse.json({ error: "Failed to create blog" }, { status: 500 }));
  }
}
