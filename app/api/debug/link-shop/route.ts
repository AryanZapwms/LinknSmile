...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!user) {
      ...
      return withCORS(NextResponse.json({ success: false, message: `User ${email} not found` }, { status: 404 }));
    }
    ...
    return withCORS(NextResponse.json({ success: false, error: error.message, stack: error.stack }, { status: 500 }));
  }
}
