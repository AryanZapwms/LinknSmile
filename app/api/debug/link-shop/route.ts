...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!user) {
      ...
      return withCORS(
        NextResponse.json({ success: false, message: `User ${email} not found` }, { status: 404 })
      );
    }
    ...
    if (!shop) {
      ...
      console.log(
        "Debug Link-Shop: Sample shops in DB:",
        allShops.map((s) => ({ id: s._id, owner: s.ownerId, name: s.shopName }))
      );
