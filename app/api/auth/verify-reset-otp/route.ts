...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!user) {
      ...
      return withCORS(NextResponse.json({ error: "User not found" }, { status: 400 }))
    }
    
    if (!user.resetOtpHash) {
      // console.log("[VERIFY_RESET_OTP] No OTP hash stored")
      return withCORS(NextResponse.json({ error: "No OTP requested for this email" }, { status: 400 }))
    }
    
    if (!user.resetOtpExpires) {
      // console.log("[VERIFY_RESET_OTP] No OTP expiry")
      return withCORS(NextResponse.json({ error: "OTP expired" }, { status: 400 }))
    }
    ...
    // 
    if (!isValid) {
      return withCORS(NextResponse.json({ error: "Incorrect OTP" }, { status: 400 }))
    }
    ...
    return withCORS(NextResponse.json({ error: "Something went wrong: " + String(err) }, { status: 500 }))
  }
}
