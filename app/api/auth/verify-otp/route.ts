...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!email || !otp) {
      return withCORS(NextResponse.json({ error: "Email and otp required" }, { status: 400 }));
    }
    ...
    if (existingVerifiedUser) {
      ...
      return withCORS(NextResponse.json({ message: "Email already verified" }, { status: 200 }));
    }

    const otpDoc = await Otp.findOne({ email: normalizedEmail }).sort({ createdAt: -1 });
    if (!otpDoc) {
      return withCORS(NextResponse.json({ error: "No OTP found" }, { status: 400 }));
    }
    ...
    if (otpHash !== otpDoc.otpHash) {
      ...
      await otpDoc.save();
      return withCORS(NextResponse.json({ error: "Invalid OTP" }, { status: 400 }));
    }
